...
import {getRedisCacheByKey, redis_client} from '../db/redis/redis'
...
const getAllBusesKey = 'getAllBuses';
/**
   * Get All Buses
   * @param {object} req 
   * @param {object} res 
...
const getAllBuses = async (req, res) => {
  try {
    ...
    const busesCache = await getRedisCacheByKey(getAllBusesKey, 60);
    const busesCacheData = JSON.parse(busesCache);
    console.log('%c data in cache', 'font-size:20px;color:lime;', busesCacheData);
    successMessage.data = busesCacheData;
    return res.status(status.success).json(successMessage);
  }
  catch(e) {
    console.log('%c e in cache', 'font-size:20px;color:lime;', e);
  }
  ...
  try {
    ...
    redis_client.setex(getAllBusesKey, 60, JSON.stringify(dbResponse))
    successMessage.data = dbResponse;
